	 ...
	 do
        {
            ...
            case 3:
                ...
                break;
            case 4:
				mando=Imprimir(&primero, &ultimo, numero_Imp);
                AgregarDoc(&DocPrim, &DocUlt, &ultimoUS, &mando);
                impresoras_Conectadas(&primero, &DocPrim, &ultimoUS, &mandado, numero_Imp);	
                system("pause");
                break;
            case 5:
				ingresar_Usuario(&primerUS,&ultimoUS);
                system("pause");
                break;
            case 6:
                printf("¡Adios!\n");
...
int AgregarDoc(Nodo **DocPrim, Nodo **DocUlt, Usuario **ultimoUS, Imp **mando){
	...
	Usuario *UsActual;
	UsActual=*ultimoUS;
	strcpy(NewDoc->usuario, UsActual->usuario);//para saber quien mando a imprimir el documento
	NewDoc->prioridad = UsActual->prioridad;//para conocer la prioridad del documento


	Nodo *nodoD;
	nodoD=*DocPrim;

    (NewDoc->mandado_a_imprimir)=(*mando);//ahora mandado a imprimir apunta a la impresora a la que se veio el documento

	if(*DocUlt){//si ya existen documentos
    while((nodoD->siguiente)!=NULL){
            while(NewDoc->prioridad >= nodoD->prioridad && NewDoc->prioridad >= nodoD->siguiente->prioridad && (nodoD->siguiente)!=NULL )
            {
                nodoD=nodoD->siguiente;
                if((nodoD->siguiente)==NULL){
                break;
                }
            }

    }
		NewDoc->cta = ((*DocUlt)->cta)++; //aumentamos contador de documentos
		if(NewDoc->prioridad < nodoD->prioridad)
        {
            NewDoc->siguiente=nodoD;//pone antes
        }
        else{
		NewDoc->siguiente=nodoD->siguiente;
		nodoD->siguiente=NewDoc;
        }
		printf("Ingrese el nombre del archivo a imprimir:\n");
		fflush(stdin);
		gets(NewDoc->docu);//leemos el nombre del nuevo documento
	}
	
	if(NewDoc->siguiente==NULL){
	*DocUlt = NewDoc;
    }

	//caso para el primer documento a ingresar
	if(!*DocPrim){
		*DocPrim = NewDoc;//el apuntador DocPrimero apunta a la nueva estructura de documentos
		printf("Ingrese el nombre del archivo a imprimir:\n");
		fflush(stdin);
		gets(NewDoc->docu);//leemos el nombre del nuevo documento
		NewDoc->siguiente=NULL;
		(*DocUlt)->cta++;//aumentamos el contador de doccumentos
		//NOTA: para este caso el primero y ultimo apuntan a la misma estructura
	}
	return 0;
...
int ingresar_Usuario(Usuario **primerUS, Usuario **ultimoUS){
	    fflush(stdin);
    do{
        printf("Introduzca la contraseña para el acceso: \n");
        gets(claveUsuario);
        if(strcmp(claveSitema,claveUsuario)!=0){
            printf("La contraseña es incorrecta \n\n");
        }
    }
    while(strcmp(claveSitema,claveUsuario)!=0);

    Usuario *nuevoUs;

    if((nuevoUs=(Usuario *)malloc(sizeof(Usuario)))==NULL){//apartamos memoria para la nueva estructura de documentos
		printf("\nNo puede almacenar memoria\n Reinicie el programa");
		return -1;
		}
    nuevoUs->siguiente = NULL;

	if(*ultimoUS){
		nuevoUs->noUs = ((*ultimoUS)->noUs)++;
		(*ultimoUS)->siguiente = nuevoUs;
		printf("\nIngrese el nombre del usuario: ");
		fflush(stdin);
		gets(nuevoUs->usuario);
		printf("\nIngrese la prioridad de %s: ",nuevoUs->usuario);
		scanf("%d", &(nuevoUs->prioridad));
	}

	*ultimoUS = nuevoUs;
	
		//caso para el primer usuario
	if(!*primerUS){
		*primerUS = nuevoUs;
		printf("\nIngrese el nombre del primer usuario: ");
		fflush(stdin);
		gets(nuevoUs->usuario);
		printf("\nIngrese la prioridad de %s: ",nuevoUs->usuario);
		scanf("%d", &(nuevoUs->prioridad));
